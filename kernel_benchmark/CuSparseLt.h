...
void transform_cusparselt(half* A_h, half* A1_h, half* A2_h, int length)
{
...
void check_A1_h_cusparselt(half* A, int length)
{
    assert(length % 4 == 0);
...
int CuSparseLt_matmul(half* A_h, half* B_h, half* C_h, int m, int n, int k, float* milliseconds)
{
    //
    half *A1_h, *A2_h;
    ...
    transform_cusparselt(A_h, A1_h, A2_h, m * k);
    free(A2_h);
    // check_A1_h_cusparselt(A1_h, m*k);
    half *A1_d, *B_d, *C_d, *D_d;
    ...
    CHECK_CUDA(cudaMalloc((void**)&D_d, m * n * sizeof(half)))
    ...
    CHECK_CUDA(cudaMemset(D_d, 0, m * n * sizeof(half)))
    ...
    CHECK_CUSPARSE(cusparseLtMatmulSearch(
        &handle, &plan, &alpha_sparTA, A1_d_compressed, B_d, &beta_sparTA, C_d, D_d, nullptr, nullptr, 0))
    //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    for (int i = 0; i < WARM_UP_ITERATION; i++) {
        CHECK_CUSPARSE(cusparseLtMatmul(
            &handle, &plan, &alpha_sparTA, A1_d_compressed, B_d, &beta_sparTA, C_d, D_d, workspace_d, nullptr, 0))
    }
    ...
    *milliseconds = *milliseconds / BENCHMARK_ITERATION;
    ...
    cudaFree(A1_d);
    cudaFree(B_d);
    cudaMemcpy(C_h, D_d, sizeof(half) * m * n, cudaMemcpyDeviceToHost);  // Col Major
    cudaFree(C_d);
    return 0;
}